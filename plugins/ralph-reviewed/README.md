...
- State file is at repo root: `{GIT_ROOT}/.rl/state.json`
...
- Check `.rl/state.json` has `"review_enabled": true`
...
- Increase via `~/.claude/codex.json`: `"timeout_seconds": 1500`
- The hook clamps `timeout_seconds` to `[60, 1680]` so it always finishes before the 1800s `Stop` hook timeout. Values outside that range are silently clamped (look for `WARNING: Clamping timeout` in the debug log).
